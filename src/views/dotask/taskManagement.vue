...
<template>
  <main class="container" :class="['drawer-' + drawerDirection]">
    ...
    />
    <!-- 新手指引组件 -->
    <GuidedTour ref="guidedTourRef" @guide="handleGuide" />
  </main>
  ...
  <div
    ...
    :class="[
      ...
      { show: detailDrawerVisible, 'no-anim': noAnim }
    ]"
...
const getProcessList = async () => {
  ...
  // 获取到所有流程状态并定位到第一个流程
  const allSchedulesList = rows.map(item => item.scheduleList.split(",")).flat()
  ...
  // 将id和username映射保存起来
  allLeadersMap.forEach((value, key) => {
    userIdUsernameMap.value[key] = value.label
  })
  localStorage.setItem(
    "userIdUsernameMap",
    JSON.stringify(userIdUsernameMap.value)
  )
...
const loadAllTasks = async () => {
  try {
    ...
    taskRows.forEach(task => {
      ...
      if (task?.scheduleList) {
        ...
        if (scheduleArray && scheduleArray.length === 1) {
          ...
          task.isFinalSchedule =
            task.schedule === scheduleArray[scheduleArray?.length - 1]
        }
      }
    ...
    try {
      ...
      const allLoadedTasks = filteredAllSchedules.value.flatMap(
        s => s.taskList || []
      )
      const overdueTasks = allLoadedTasks.filter(t => isTaskOverdue(t.deadline))
      localStorage.setItem(
        "overdue_tasks",
        JSON.stringify(overdueTasks.map(t => t.id))
      )
...
const computeDrawerSide = async () => {
  try {
    ...
    // 对获取到的位置和尺寸先除以缩放因子
    const adjustedPos = originalWindowPos.value || {
      x: pos.x / scaleFactor,
      y: pos.y / scaleFactor
    }
    const adjustedSize = originalWindowSize.value || {
...
const expandMainWindowForDrawer = async () => {
  try {
    ...
    const adjustedCurrentPos = {
      x: currentPos.x / scaleFactor,
      y: currentPos.y / scaleFactor
    }
    const adjustedCurrentSize = {
    ...
    if (!originalWindowPos.value || !originalWindowSize.value) {
      originalWindowPos.value = {
        x: adjustedCurrentPos.x,
        y: adjustedCurrentPos.y
      }
      originalWindowSize.value = {
        width: adjustedCurrentSize.width,
        height: adjustedCurrentSize.height
      }
    }
    ...
    if (
      adjustedCurrentSize.width !== targetWidth ||
      adjustedCurrentPos.x !== targetX
    ) {
  ...
  try {
    ...
    const targetX =
      drawerDirection.value === "ltr"
        ? adjustedCurrentPos.x + dw
        : adjustedCurrentPos.x
...
const handleInlineSaved = async params => {
  ...
  if (params?.id) {
    ...
    if (
      params.parentId === 0 &&
      ((active_process.value === "all" &&
        active_alltask.value !== params.schedule) ||
        (active_process.value !== "all" &&
          params.schedule !== active_onetask.value))
    ) {
...
const handleAddOrUpdateSubTask = async subTaskData => {
  ...
  let params = {
    ...subTaskData,
    deadline: isFormattedDeadline
      ? subTaskData.deadline
      : formatToLocalTime(subTaskData.deadline) || null
  }
...
const handleProjectSelected = async project => {
  ...
  await handleProject({
    index:
      projectList.value.findIndex(p => p.projectName === project.projectName) +
      1
  })
}
...
const updateTaskRecursively = (taskList, updatedTask, updateUserArray) => {
  return taskList.map(task => {
    ...
    if (task.children && task.children.length > 0) {
      return {
        ...task,
        children: updateTaskRecursively(
          task.children,
          updatedTask,
          updateUserArray
        )
...
.inline-detail-panel.side-rtl {
  right: 0;
  transform: translateX(100%);
}
.inline-detail-panel.side-ltr {
  left: 0;
  transform: translateX(-100%);
}

.container.drawer-ltr .tabs-container {
  margin-left: auto;
}
.inline-detail-panel.show {
  width: var(--drawer-size, 620px);
