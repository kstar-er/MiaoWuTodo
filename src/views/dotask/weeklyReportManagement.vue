...
watch(
  () => props.reportChanged,
  (newValue, oldValue) => {
    console.log("周报按钮已切换", newValue, oldValue)
    if (props.reportChanged) {
      activeTab.value = "templateManagement"
      handleTabChange("reportList")
    }
  }
)
const handleTabChange = tabName => {
  // 切换标签页时，可以重新加载对应数据
  proxy.$emit("updateReportTab", tabName)
...
.container.drawer-ltr .weekly-report-container {
  margin-left: auto;
}
